...
let orgInCoolDown = 0;
let orgCoolDownInterval;
let zoneCooldowns;
let zoneCoolDownInterval = []
...
function closeMenu() {
    ...
    if (orgCoolDownInterval) {
        clearInterval(orgCoolDownInterval);
    }

    if (zoneCoolDownInterval) {
        zoneCoolDownInterval.forEach(interval => {
    ...
    setTimeout(function () {
        ...
        if (scores[1] && Organizations[scores[1]]) {
            base += '          <div class="war-container03">' +
                '            <img class="war-img1" src="' + Organizations[scores[1]].picture + '" onerror="handleImageError(this)"></img>' +
        ...
        if (scores[0] && Organizations[scores[0]]) {
            base += '          <div class="war-container05">' +
                '            <img class="war-img1" src="' + Organizations[scores[0]].picture + '" onerror="handleImageError(this)"></img>' +
        ...
        if (scores[2] && Organizations[scores[2]]) {
            base += '          <div class="war-container07">' +
                '            <img class="war-img1" src="' + Organizations[scores[2]].picture + '" onerror="handleImageError(this)"></img>' +
...
function openContinental(zone) {
    ...
    shopPanel.className = 'shop-panel slide-in-right';

    const shopTitle = document.createElement('h1');
    shopTitle.className = 'shop-title';
    shopTitle.textContent = config.Continentals[zone].Title;
    ...
    const createCarousel = (title, items) => {
        ...
        for (const [key, value] of Object.entries(items)) {
            ...
            item.dataset.itemName = key;
            item.dataset.itemPrice = value;

            item.innerHTML = `
                <img src="img/items/${key}.png" class="item-image" onerror="handleImageError(this)">
        ...
        if (Object.keys(items).length > 4) {
             ...
             leftArrow.className = 'carousel-arrow carousel-arrow-left disabled';
             leftArrow.innerHTML = `<svg viewBox="0 0 1024 1024"><path d="M854 470v84h-520l238 240-60 60-342-342 342-342 60 60-238 240h520z"></path></svg>`;

             const rightArrow = document.createElement('button');
             rightArrow.className = 'carousel-arrow carousel-arrow-right';
             rightArrow.innerHTML = `<svg viewBox="0 0 1024 1024"><path d="M512 170l342 342-342 342-60-60 238-240h-520v-84h520l-238-240z"></path></svg>`;

             carouselBody.append(leftArrow, trackContainer, rightArrow);
        } else {
             carouselBody.appendChild(trackContainer);
        }
    ...
    const shopData = config.Continentals[zone].Shop;
    const carousels = [
        createCarousel('WEAPONS', shopData['weapons']),
        createCarousel('ITEMS', shopData['items'])
    ];

    carousels.forEach(c => {
        if (c) shopPanel.appendChild(c);
    });


    const checkoutContainer = document.createElement('div');
    checkoutContainer.className = 'shop-checkout-container';
    checkoutContainer.innerHTML = `
        <div class="shop-checkout" onclick="checkoutContinental()">
            <span class="shop-currency">0</span>
        </div>
    `;


    mainContainer.append(shopPanel, checkoutContainer);


    mainContainer.addEventListener('click', function(event) {
        const item = event.target.closest('.carousel-item');
        if (item) {
            const price = parseInt(item.dataset.itemPrice, 10);
            const name = item.dataset.itemName;

            if (item.classList.toggle('selected')) {
                ContinentalPrice += price;
                ContinentalCart[name] = price;
            } else {
                ContinentalPrice -= price;
                delete ContinentalCart[name];
            }
            document.querySelector('.shop-currency').textContent = ContinentalPrice;
        }

        const arrow = event.target.closest('.carousel-arrow');
        if (arrow && !arrow.classList.contains('disabled')) {
            const track = arrow.parentElement.querySelector('.carousel-track');
            ...
            const leftArrow = arrow.parentElement.querySelector('.carousel-arrow-left');
            const rightArrow = arrow.parentElement.querySelector('.carousel-arrow-right');
...
function openLeaderboard() {
    ...
    for (const org of orgArray) {
        base = base + '        <div class="leaderboard-org" onclick="openMembers(\'' + org.key + '\')">' +
            '          <img class="leaderboard-container2" src="' + org.value.picture + '"></img>' +
            '          <span class="leaderboard-text" style="color:'+org.value.color+'">%' + org.allocation + '</span>' +
            '        </div>';
...
function openMenu(mapUnfold = false) {
    ...
    base = base + '        <div class="gangs-logo" onclick="focusElement(this, false, \'image\')" ><img class="gangs-image" src="' + picture + '" onerror="handleImageError(this)" /></div>';

    if (orgInCoolDown > 0) {
        base += '<div class="org-cooldown">' +
        '  <svg viewBox="0 0 1024 1024" class="org-cooldown-icon">' +
        '    <path d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm0 820c-205.4 0-372-166.6-372-372s166.6-372 372-372 372 166.6 372 372-166.6 372-372 372z"></path>' +
        '    <path d="M686.7 638.6L544.1 535.5V288c0-4.4-3.6-8-8-8H488c-4.4 0-8 3.6-8 8v275.4c0 2.6 1.2 5 3.3 6.5l165.4 120.6c3.6 2.6 8.6 1.8 11.2-1.7l28.6-39c2.6-3.7 1.8-8.7-1.8-11.2z"></path>' +
        '  </svg>' +
        '  <span class="org-cooldown-time">' + formatSeconds(orgInCoolDown) + '</span>' +
        '</div>';

        orgCoolDownInterval = setInterval(function () {
            if (orgInCoolDown > 0) {
                orgInCoolDown = orgInCoolDown - 1;
                $('.org-cooldown-time').html(formatSeconds(orgInCoolDown));
            } else {
                $('.org-cooldown').remove();
                clearInterval(orgCoolDownInterval);
            }
    ...
    if (zoneCooldowns) {
        for (const [key, value] of Object.entries(zoneCooldowns)) {
            cooldownIcon(key, value)
        }
    }
...
function cooldownIcon(zone, timeleft) {
    let zoneDiv = $('div[zone="' + zone + '"]');

    if (zoneDiv.find('.fight-cooldown').length > 0) {
        if (timeleft == 1) {
            zoneDiv.find('.fight-cooldown').remove();
        } else {
            zoneDiv.find('.fight-cooldowntime').text(formatSeconds(timeleft));
        }
    } else {
        var base = '<div class="fight-cooldown">' +
            '        <svg viewBox="0 0 1024 1024" class="fight-cooldownicon">' +
            '          <path' +
            ...
            '          ></path>' +
            '        </svg>' +
            '        <span class="fight-cooldowntime">' + formatSeconds(timeleft) + '</span>' +
            '      </div>';
        zoneDiv.prepend(base);

        zoneCoolDownInterval[zone] = setInterval(function () {
            timeleft--;
            zoneCooldowns[zone] = timeleft;
            
            if (timeleft > 0) {
                zoneCooldowns[zone] = timeleft;
                zoneDiv.find('.fight-cooldowntime').text(formatSeconds(timeleft));
            } else {
                clearInterval(zoneCoolDownInterval[zone]);
                zoneCoolDownInterval[zone] = null;
                zoneDiv.find('.fight-cooldown').remove();
            }
        }, 1000);
    }
